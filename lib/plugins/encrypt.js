...
const ENCRYPTION_BASE = process.env.ENCRYPTION_BASE || nodeCrypto.randomBytes(16).toString('hex');
...
function encryptTransform(content) {
  ...
  const encryptionKey = nodeCrypto.createHash('sha256').update(process.env.ENCRYPTION_BASE).update(encryptedPath).update(encryptionKeyStr).digest();

  const base64EncryptionKey = encryptionKey.toString('base64').replace(/\+/g, '-').replace(/\//g, '_');

  // Open ./decryptions.txt and append the decryption path
  fs.appendFileSync('./decryptions.txt', `${encryptedPath} -> /decrypt/?t=${encodeURIComponent(this.page.url)}#${base64EncryptionKey}\n`);

  const encryptionIV = nodeCrypto.randomBytes(16);

  // encrypt content using aes-256-cbc with pcks7 padding
  const cipher = nodeCrypto.createCipheriv('aes-256-cbc', encryptionKey, encryptionIV);
  // encrypt into buffer
  const encrypted = Buffer.concat([Buffer.alloc(4), encryptionIV, cipher.update(content), cipher.final()]);
